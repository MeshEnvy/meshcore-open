...
class AssetEncoder {
  ...
  static Uint8List decode({
    ...
    debugPrint('AssetEncoder.decode: blobBytes.length=${blobBytes.length}');
    final blob = AssetBlob.fromBuffer(blobBytes);
    debugPrint(
      'AssetEncoder.decode: type=${blob.type}, contentType=${blob.contentType}, filename=${blob.filename}, recipients=${blob.recipients.length}',
    );
    Uint8List? key;

    if (blob.type == AssetType.DM) {
      debugPrint(
        'AssetEncoder.decode: DM mode, recipients=${blob.recipients.length}',
      );
      if (blob.recipients.isNotEmpty) {
        ...
        debugPrint(
          'AssetEncoder.decode: Decracted key from recipient data, length=${key.length}',
        );
      }
    } else if (blob.type == AssetType.CHANNEL) {
      debugPrint(
        'AssetEncoder.decode: Channel mode, using sharedPsk (present=${sharedPsk != null})',
      );
      key = sharedPsk;
    }
    ...
    final cipherText = Uint8List.fromList(blob.encryptedData.sublist(ivLength));

    debugPrint(
      'AssetEncoder.decode: key.length=${key.length}, keyHex=${_bytesToHex(key)}, cipherText.length=${cipherText.length}',
    );
    final result = _decryptAesGcm(key, iv, cipherText);
    debugPrint('AssetEncoder.decode: decryptedData.length=${result.length}');
    return result;
  }
}
